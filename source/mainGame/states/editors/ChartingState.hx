package mainGame.states.editors;
...
import mainGame.objects.Note;
import mainGame.objects.StrumNote;
import mainGame.objects.NoteSplash;
import mainGame.objects.HealthIcon;
import mainGame.objects.AttachedSprite;
import mainGame.objects.sprites.Character;
import mainGame.substates.Prompt;
...
class ChartingState extends MusicBeatState
{
	...
	[
	    ...
	    ['---VEMER ENGINE---',"for categories."],
		['camZoom',"Value 1: Set your zoom.\n Value 2:speed zoom (not necessarily)"],
		['CameraSpeed',"Value 1: Set your camera speed."],
		['Lyrics',"Value 1: Lyrics\nValue 2: Color (white is default)"],
		['BeatCamera',"Value 1:Your number beat \nValue 2: Multiplier (1 is default) \nValue 3:What the Camera only Beat? \nExample:[CamHud] [camGame] [empty - both] \nIf you want turn off just leave Value 1 and 2 empty"],
		['Move Camera From Notes',"Value 1: [1]=turn on [2]=turn off "],
		['Fade HUD',"Value 1: alpha HUD \n Value 2:How much time he gona gone or show"],
		['--not done--',"am not make it \n -VEMER"],
		['set Shaders', '1 amount, 2 tween speed'],
		['Shader Beat', '1 amount, 2 interval'],
		['Play Video',"value 1: Video directory name. \nValue 2: set on Camera video"],
		['',""],
		['---MODS---',"for categories."],
		['FadeBemonBG',"Value 1:Set alpha.\nValue 2: Set How Much Time \n only stage: Bemon"]
	];
	...
	**/
	...
	public static var textQuid:Bool = true;
	...
	override function create()
	{
		...
		//Main.appDetail = "Charting Menu";
		//Main.appDopPrefix = " / ";
		//Main.appDopDetail = Paths.formatToSongPath(_song.song);
		...
		textQuid = FlxG.save.data.chart_textQuid;
		ignoreWarnings = FlxG.save.data.ignoreWarnings;
		bg = new FlxSprite().loadGraphic(Paths.image('Menu/Chart/Bg'));
		...
		FlxTween.tween(bg, {alpha: 1}, 5, {ease: FlxEase.quartInOut});
		...
		for (i in 0...tipTextArray.length) {
			tipText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 8, 0, tipTextArray[i], 16);
			tipText.y += i * 12;
		...
		zoomTxt = new FlxText(10, 150, 0, "Zoom: 1 / 1", 16);
	...
	var check_textQuid:FlxUICheckBox = null;
	...
	function addSongUI():Void
	{
		...
		var player4DropDown = new FlxUIDropDownMenu(player1DropDown.x, player2DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
			...
			player4DropDown.selectedLabel = _song.player2;
		...
		tab_group_song.add(new FlxText(player4DropDown.x, player4DropDown.y -15, 0, 'Mom:'));
	...
	var check_momSection:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_momSection = new FlxUICheckBox(10, check_gfSection.y + 22, null, null, "Mom section", 100);
		check_momSection.name = 'check_mom';
		check_momSection.checked = _song.notes[curSec].gfSection;
		...
		tab_group_section.add(check_momSection);
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(20, 215, 0, eventStuff[0][0]);
		...
		tab_group_event.add(text);
		value1InputText = new FlxUIInputText(20, 110, 230, "");
		blockPressWhileTypingOn.push(value1InputText);

		var text:FlxText = new FlxText(20, 130, 0, "Value 2:");
		tab_group_event.add(text);
		value2InputText = new FlxUIInputText(20, 150, 230, "");
		blockPressWhileTypingOn.push(value2InputText);

		var text:FlxText = new FlxText(20, 170, 0, "Value 3:");
		tab_group_event.add(text);
		value3InputText = new FlxUIInputText(20, 190, 230, "");
	...
	function addChartingUI() {
		...
		check_textQuid = new FlxUICheckBox(130, 200, null, null, "Show text Quide", 100);
		if (FlxG.save.data.chart_textQuid == null) FlxG.save.data.chart_textQuid = true;
		check_textQuid.checked = FlxG.save.data.chart_textQuid;

		check_textQuid.callback = function()
		{
				FlxG.save.data.chart_textQuid = check_textQuid.checked;
				textQuid = FlxG.save.data.chart_textQuid;
		};
		...
		tab_group_chart.add(check_textQuid);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Mom section':
					_song.notes[curSec].momSection = check.checked;
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.save.data.chart_textQuid == true)
			tipText.visible = true;
		else if(FlxG.save.data.chart_textQuid == false)
		{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				openSubState(new mainGame.states.editors.EditorPlayState(playbackSpeed));
			}
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				MusicBeatState.switchState(new mainGame.states.editors.MasterEditorMenu());
	...
	*/
	...
	function updateSectionUI():Void
	{
		...
		check_momSection.checked = sec.momSection;
