...
import mainGame.objects.HealthBar;
...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		leHealthIcon.y = healthBar.y - 75 + char.iconPosition[1];
		leHealthIcon.x = healthBar.barCenter - (150 * leHealthIcon.scale.x) / 2 - 26 * 2 + char.iconPosition[0];
		add(leHealthIcon);
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.iconPosition = parsedJson.icon_position;
	...
	var positionXIconLol:FlxUINumericStepper;
	var positionYIconLol:FlxUINumericStepper;
	...
	function addIconsUI() {
		...
		//position icons
		tab_group.add(new FlxText(15, 15, 0, 'Icon X/Y:'));

		positionXIconLol = new FlxUINumericStepper(25, 35, 10, char.iconPosition[0], -9000, 9000, 0);
		positionYIconLol = new FlxUINumericStepper(85, 35, 10, char.iconPosition[1], -9000, 9000, 0);

		tab_group.add(positionXIconLol);
		tab_group.add(positionYIconLol);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == positionXIconLol)
			{
				char.iconPosition[0] = positionXIconLol.value;
				updatePostionIcon();
			}

			else if(sender == positionYIconLol)
				{
					char.iconPosition[1] = positionYIconLol.value;
					updatePostionIcon();
				}
	...
	var superYicon:Int = 600;

	function updatePostionIcon() {
		char.iconPosition[0] = positionXIconLol.value;
		char.iconPosition[1] = positionYIconLol.value;
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			positionXIconLol.value = char.iconPosition[0];
			positionYIconLol.value = char.iconPosition[1];
	...
	function saveCharacter() {
		var json = {
			...
			"icon_position": char.iconPosition,
			"icon_type": char.iconManyType,
