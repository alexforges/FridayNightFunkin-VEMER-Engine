...
class HealthBar extends FlxSpriteGroup
{
	...
	private var displayedHealth2:Float = 1;
	...
	public function new(x:Float, y:Float, image:String = 'healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1, antialiasingLol:Bool = false)
	{
		super(x, y);
		...
		for(i in [
			...
			i.antialiasing = antialiasingLol;
	...
	override function update(elapsed:Float) {
		...
		displayedHealth = FlxMath.lerp(displayedHealth, leftBar.width, .2/(FlxG.updateFramerate / 60));
		displayedHealth2 = FlxMath.lerp(displayedHealth, rightBar.width, .2/(FlxG.updateFramerate / 60));
	}
